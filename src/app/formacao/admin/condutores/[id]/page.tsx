...
export default function CondutorDetailPage() {
  ...
  // A sala medida, não `formacao_meet_presencas`. A tela antiga fazia média de
  // `media_participantes` (presença simultânea) sob o rótulo "Média", enquanto
  // o bloco logo abaixo somava `total_participantes` (cabeças) com o mesmo
  // rótulo, três linhas adiante. Agora é uma definição só, e ela exclui quem
  // conduz.
  const { sala } = useSala();
  const medido = useMemo(
    () => (condutor ? indexarCondutores(sala).get(chaveTexto(condutor.nome)) : undefined),
    [sala, condutor],
  );
  const encontrosDele = useMemo(
    () =>
      condutor
        ? (sala?.encontros ?? []).filter((e) =>
            e.condutores.some((n) => chaveTexto(n) === chaveTexto(condutor.nome)),
          )
        : [],
    [sala, condutor],
  );
      ...
      {medido && (
        <div className="rounded-xl p-5 space-y-4" style={{ background: "rgba(108,92,231,0.04)", border: "1px solid rgba(108,92,231,0.12)" }}>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <h2 className="text-sm font-fraunces font-semibold text-[#FDFBF7]">Presença medida na sala</h2>
            <span className="text-[10px] text-[#FDFBF7]/30 font-dm">
              quem entrou no Meet, sem contar quem conduz
            </span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="text-center">
              <p className="text-2xl font-fraunces font-bold text-[#FDFBF7]">{medido.encontros}</p>
              <p className="text-[10px] text-[#FDFBF7]/35 font-dm">Encontros</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-fraunces font-bold text-[#FDFBF7]">
                {medido.quorumMedio?.toFixed(1) ?? "—"}
              </p>
              <p className="text-[10px] text-[#FDFBF7]/35 font-dm">Pessoas por encontro</p>
            </div>
            <div className="text-center">
              <p
                className="text-2xl font-fraunces font-bold"
                style={{
                  color:
                    medido.tendencia === null
                      ? "#FDFBF7"
                      : medido.tendencia >= 0
                        ? "#2E9E8F"
                        : "#E07A5F",
              ...
              >
                {medido.tendencia === null
                  ? "—"
                  : `${medido.tendencia > 0 ? "+" : ""}${medido.tendencia.toFixed(1)}`}
              </p>
              <p className="text-[10px] text-[#FDFBF7]/35 font-dm">
                {medido.tendencia === null ? "Faltam encontros" : "Tendência"}
              </p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-fraunces font-bold text-[#FDFBF7]">
                {medido.vozesAtivasPct !== null ? `${medido.vozesAtivasPct.toFixed(0)}%` : "—"}
              </p>
              <p className="text-[10px] text-[#FDFBF7]/35 font-dm">
                Falaram{medido.encontrosComTranscricao > 0 ? ` (${medido.encontrosComTranscricao} c/ transcrição)` : ""}
              </p>
            </div>
          </div>

          {/* Zero em fala do condutor quer dizer "não reconheci quem conduzia",
              não "ficou calado". Por isso a ausência é dita, não desenhada
              como zero. */}
          <p className="text-[11px] text-[#FDFBF7]/40 font-dm">
            {medido.falaCondutorPct !== null
              ? `Fala de ${condutor?.nome.split(" ")[0]}: ${medido.falaCondutorPct.toFixed(0)}% do tempo falado, medido em ${medido.encontrosComCondutorReconhecido} ${medido.encontrosComCondutorReconhecido === 1 ? "encontro" : "encontros"}.`
              : "Fala do condutor ainda não medida: a ingestão não reconheceu quem conduzia nesses encontros."}
          </p>

          {encontrosDele.length > 0 && (
            <div className="space-y-1.5 pt-2" style={{ borderTop: "1px solid rgba(108,92,231,0.1)" }}>
              <p className="text-[10px] text-[#FDFBF7]/30 font-dm uppercase tracking-wider">
                Encontro a encontro
              </p>
              {encontrosDele.map((e) => (
                <div key={e.id} className="flex items-center justify-between gap-2 px-2 py-1.5 rounded-lg" style={{ background: "rgba(255,255,255,0.02)" }}>
                  <span className="font-dm text-xs text-[#FDFBF7]/60 truncate">
                    {new Date(e.data + "T12:00:00").toLocaleDateString("pt-BR", { day: "2-digit", month: "short" })}
                    {" · "}
                    {e.atividade ?? "Sem atividade"}
                    {e.condutores.length > 1 && (
                      <span className="text-[#FDFBF7]/30"> · com {e.condutores.filter((n) => chaveTexto(n) !== chaveTexto(condutor?.nome ?? "")).join(", ")}</span>
                    )}
                  </span>
                  <span className="font-dm text-xs text-[#FDFBF7]/40 shrink-0">
                    {e.quorum} {e.quorum === 1 ? "pessoa" : "pessoas"}
                  </span>
                </div>
              ))}
            </div>
