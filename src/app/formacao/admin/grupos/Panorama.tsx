...
export default function Panorama() {
  ...
  /**
   * A lista é o CATÁLOGO, não a sala.
   *
   * Treze das dezenove atividades não têm posição na grade, e elas carregam 185
   * feedbacks. Uma lista que só mostrasse quem teve encontro medido esconderia
   * dois terços do que existe, e o grupo que saiu da grade em maio ficaria
   * inalcançável para sempre.
   */
  const catalogo = useMemo(() => {
    const medidoPorId = new Map(
      grupos.filter((g) => g.atividadeId).map((g) => [g.atividadeId as string, g]),
    );
    const q = busca.trim().toLowerCase();
    return (sala?.catalogo ?? [])
      .filter((a) => (verArquivados ? a.arquivado : !a.arquivado))
  ...
  const totalEmUso = (sala?.catalogo ?? []).filter((a) => !a.arquivado).length;
  const totalArquivados = (sala?.catalogo ?? []).length - totalEmUso;
  ...
  async function criarGrupo() {
    ...
    setCriando(true);
    try {
      const r = await fetch("/formacao/api/admin/grupos", {
        method: "POST",
      ...
      setNomeNovo("");
      router.push(`/formacao/admin/grupos/${j.atividade.id}`);
    } catch (e) {
  ...
  return (
    <div className="space-y-4 pb-10">
      ...
      {/* O aviso de sala vazia não pode mais esconder a lista: o catálogo tem
          dezenove grupos e a sala mede seis. Sem isso, um período sem encontro
          capturado apagava da tela treze grupos que existem. */}
      {vazio && (
        <Card>
          <p className="font-dm text-sm text-cream/50 leading-relaxed">
            ...
            encontro de sala configurada. Os grupos continuam abaixo, sem medição.
          </p>
        </Card>
      )}
      {!vazio && (
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}>
          <Card>
            ...
            <div className="flex flex-col sm:flex-row gap-2 mb-4">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="dark-input flex-1 sm:w-52 rounded-[10px] px-3 py-2.5 font-dm text-base sm:text-sm"
                />
                <button
                  onClick={criarGrupo}
            ...
            <div className="flex gap-2 mb-3 flex-wrap">
              {([[false, "Em uso", totalEmUso], [true, "Arquivados", totalArquivados]] as const).map(
                ([v, rotulo, n]) => (
                  <button
                    key={rotulo}
                    onClick={() => setVerArquivados(v)}
                    className="font-dm text-[11px] px-3 py-1.5 rounded-full transition-all"
                    style={{
                      background: verArquivados === v ? "rgba(200,75,49,0.12)" : "rgba(255,255,255,0.03)",
                      color: verArquivados === v ? TERRACOTA : "rgba(253,251,247,0.35)",
                      border: `1px solid ${verArquivados === v ? "rgba(200,75,49,0.3)" : "rgba(255,255,255,0.06)"}`,
                    }}
                  >
                    {rotulo} {n}
                  </button>
            ...
            <div className="space-y-1.5">
              {catalogo.length === 0 && (
                <p className="font-dm text-xs text-cream/30 py-3">
                  {busca ? "Nenhum grupo com esse nome." : "Nenhum grupo aqui."}
                </p>
              )}
              {catalogo.map((a) => {
                const m = a.medido;
                const ir = () => router.push(`/formacao/admin/grupos/${a.id}`);
                return (
                  <div
                    key={a.id}
                    onClick={ir}
                    ...
                    className="px-3 py-2.5 rounded-[10px] cursor-pointer transition-colors hover:bg-white/[0.04]"
                    style={{ background: "rgba(255,255,255,0.02)" }}
                  >
                    <div
                    ...
                    >
                      <div className="min-w-0">
                        <p className="font-dm text-xs text-cream/80 truncate">
                          {a.nome}
                          {!a.ativo && <span className="text-cream/25"> · fora do formulário</span>}
                        </p>
                    ...
                    <div className="sm:hidden space-y-1">
                      <p className="font-dm text-xs text-cream/80">{a.nome}</p>
                      <p className="font-dm text-[10px] text-cream/25">
                        {m?.condutores.length
                          ? m.condutores.join(", ")
                          : m
                      ...
                      {m && (
                        <div className="flex flex-wrap gap-x-4 gap-y-1 pt-1">
                          <span className="font-dm text-[11px] text-cream/45">
                            <strong className="font-fraunces text-cream">{num(m.quorumMedio)}</strong> na sala
                          </span>
                          <span className="font-dm text-[11px] text-cream/45">
