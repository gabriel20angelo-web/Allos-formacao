// O índice do acervo de dinâmicas, agora dentro da casa dos associados.
//
// As páginas profundas (o exercício, a categoria, a tag, a trilha) continuam em
// /formacao/aprimoramento-dinamicas: elas têm link salvo e compartilhado por
// fora, e mudar o endereço delas quebraria coisa que já circula. Só o índice
// mudou de casa, porque é ele que precisava ficar ao lado do Síncrono e dos
// Eventos, sob a mesma navegação.
...
export default async function AprimoramentoPage() {
    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5">
        {counts.map((c) => (
          <Link
            key={c.slug}
            href={`/formacao/associados/aprimoramento?cat=${c.slug}`}
...
function QuickShortcuts() {
  // Atalhos pré-aplicam filtros via URL state (lido pelo ExerciseCatalog). O
  // caminho é o do próprio índice: o catálogo reescreve a query por cima da
  // rota atual, e apontar para o endereço antigo tiraria a pessoa da área.
  const shortcuts: {
    href: string;
    label: string;
    ...
    {
      href: "/formacao/associados/aprimoramento?fmt=roleplay",
      label: "Quero roleplay",
      Icon: Drama,
    },
    {
      href: "/formacao/associados/aprimoramento?fmt=reflexao",
      label: "Pra reflexão",
      Icon: BrainCog,
    },
    {
      href: "/formacao/associados/aprimoramento?fmt=discussao",
      label: "Pra discussão",
      Icon: MessagesSquare,
    },
    {
      href: "/formacao/associados/aprimoramento?cu=curado",
      label: "Só curados",
      Icon: Star,
    },
  ];
